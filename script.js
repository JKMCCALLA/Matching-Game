...
function appendNewCard(parentElement) {
  ...
  let str = ["card-down", "card-up"];
  let card_str = `<div class=${str[0]}></div> <div class=${str[1]}></div>`;
  //console.log(card_str);

  card.innerHTML = card_str;

  // Step 4: Append the card element to the parentElement, making the card element a "child".

  document.getElementById(parentElement).appendChild(card);
...
function shuffleCardImageClasses() {
  ...
  //console.log(_.shuffle(image_array)[0]);
  return _.shuffle(image_array);
}
...
function createCards(parentElement, shuffledImageClasses) {
 ...
 for (let i = 0; i < 12; i++) {
   let card = appendNewCard("card-container");
   var ran = shuffledImageClasses[i];
